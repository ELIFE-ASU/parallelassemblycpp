...
class SummarizeSolTests(unittest.TestCase):
    def write_report(self, root: Path, *, parallel: bool) -> Path:
        path = root / ("parallel/omp-4.json" if parallel else "suites/full.json")
        ...
        benchmark.write_json_report(
            ...
            candidate_execution=benchmark.ExecutionConfig(
                environment=(("OMP_NUM_THREADS", "4" if parallel else "1"),),
