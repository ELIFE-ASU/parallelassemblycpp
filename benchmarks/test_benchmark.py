...
from benchmarks import benchmark, search_profiles
...
class BenchmarkTests(unittest.TestCase):
                ...
                if "--telemetry=1" in sys.argv:
                    ...
                    telemetry = {{
                        ...
                        "counters": {{
                            ...
                            "rejected_masks": 0,
                            "matching_visits": 4,
                            "matching_bound_refresh_polls": 10,
                            "matching_bound_refreshes": 4,
                            "matching_bound_classes_pruned": 1,
                            "matching_bound_pairs_pruned": 1,
                            "matching_bound_blocks_pruned": 1,
                            "matching_bound_candidates_pruned": 1,
    ...
    def add_valid_parallel_telemetry(
        ...
        aggregate_counters.update(
            (name, counters[name])
            for name in benchmark.MATCHING_BOUND_TELEMETRY_COUNTERS
            if name in counters
        )
        empty_counters = dict.fromkeys(aggregate_counters, 0)
    ...
    def test_matching_bound_telemetry_validation_and_legacy_compatibility(self) -> None:
        with tempfile.TemporaryDirectory() as temp_directory:
            directory = Path(temp_directory)
            fixture = self.create_fixture(directory)
            executable = self.create_fake_executable(
            ...
            subprocess.run(  # noqa: S603 - executable is generated by this test.
                [str(executable), "--telemetry=1", str(fixture)], check=True
            )
            path = directory / "inputTelemetry.json"
            serial = benchmark.parse_search_telemetry(path)
            parallel = json.loads(json.dumps(serial))
            self.add_valid_parallel_telemetry(parallel)
            ...
            def records(value: dict[str, object]) -> list[dict[str, object]]:
                ...
                if "parallel" in value:
                    result.append(value["parallel"]["aggregate"]["counters"])
                    result.extend(
                        worker["counters"] for worker in value["parallel"]["workers"]
                    )
                return result

            for label, original in (("serial", serial), ("parallel", parallel)):
                with self.subTest(mode=label, case="complete"):
                    self.assertEqual(parse(original), original)
                legacy = json.loads(json.dumps(original))
                for record in records(legacy):
                    for name in group:
                        del record[name]
                with self.subTest(mode=label, case="legacy"):
                    self.assertEqual(parse(legacy), legacy)

                for index in range(len(records(original))):
                    for name in group:
                        malformed = json.loads(json.dumps(original))
                        del records(malformed)[index][name]
                        with (
                            self.subTest(mode=label, record=index, missing=name),
                            self.assertRaisesRegex(
                                benchmark.BenchmarkError, "matching-bound counters"
                            ),
                        ):
                            parse(malformed)
                        for invalid in (-1, True, 1.5, None):
                            malformed = json.loads(json.dumps(original))
                            records(malformed)[index][name] = invalid
                            with (
                                self.subTest(
                                    mode=label,
                                    record=index,
                    ...
                    if label == "parallel":
                        ...
                        for name in group:
                            del records(malformed)[index][name]
                        with (
                            self.subTest(record=index, case="missing group"),
                            self.assertRaisesRegex(
                                benchmark.BenchmarkError,
                                "matching-bound counter group",
                            ),
                        ):
                            parse(malformed)

            for name in group - {"matching_bound_refresh_polls"}:
                malformed = json.loads(json.dumps(serial))
                malformed["counters"][name] = 11
                with (
                    self.subTest(counter=name, case="impossible count"),
                    self.assertRaisesRegex(
                        benchmark.BenchmarkError,
                        "inconsistent search matching-bound counters",
                    ),
                ):
                    parse(malformed)

            for name in group:
                malformed = json.loads(json.dumps(parallel))
                malformed["parallel"]["aggregate"]["counters"][name] += 1
                with (
                    self.subTest(counter=name, case="worker sum"),
                    self.assertRaisesRegex(
                        benchmark.BenchmarkError,
                        f"aggregate counter {name} does not match workers",
                    ),
                ):
                    parse(malformed)
                malformed["parallel"]["workers"][0]["counters"][name] += 1
                with (
                    self.subTest(counter=name, case="top-level sum"),
                    self.assertRaisesRegex(
                        benchmark.BenchmarkError,
                        "aggregate counters do not match legacy telemetry",
                    ),
    ...
    def test_shared_cache_admission_telemetry_and_legacy_compatibility(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            path = directory / "inputTelemetry.json"
            telemetry = benchmark.parse_search_telemetry(path)
            self.add_valid_parallel_telemetry(telemetry)
            ...
            self.assertEqual(parse(), telemetry)
            cache.update(
                table_count=1, misses=2, lock_acquisitions=2, admission_rejections=2
            )
            self.assertEqual(parse(), telemetry)
            cache.update(
                admissions=1,
                ...
                allocated_bytes=100,
                arena_allocated_bytes=120,
                slot_bytes=64,
                admission_filter_bytes=64,
                hits=2,
                pruned_hits=1,
                updated_hits=1,
                lock_acquisitions=4,
                growth_count=2,
                rehashed_entries=3,
                growth_nanoseconds=9,
                max_growth_nanoseconds=6,
            )
            self.assertEqual(parse(), telemetry)
            for name in benchmark.SHARED_CACHE_ADMISSION_FIELDS:
                original = cache.pop(name)
                with (
                    self.subTest(missing=name),
                    self.assertRaisesRegex(
                        benchmark.BenchmarkError, "admission counters"
                    ),
                ):
                    parse()
                cache[name] = original
                for invalid in (-1, True, 1.5, None):
                    cache[name] = invalid
                    with (
                        self.subTest(counter=name, invalid=invalid),
                        self.assertRaisesRegex(
                            benchmark.BenchmarkError, "admission counters"
                        ),
                    ):
            ...
            for name, invalid, message in (
                ...
                ("pruned_hits", 0, "hit outcomes"),
                ("allocated_bytes", 0, "allocation"),
                ("arena_allocated_bytes", 99, "retained bytes exceed arena"),
                ("max_growth_nanoseconds", 10, "growth timing"),
            ):
                original = cache[name]
                cache[name] = invalid
                with (
                    self.subTest(counter=name),
                    self.assertRaisesRegex(benchmark.BenchmarkError, message),
                ):
                    parse()
                cache[name] = original

            # Older baseline artifacts above remain valid without the newer
            # layout diagnostics. New artifacts provide the entire group.
            cache.update(dict.fromkeys(benchmark.SHARED_CACHE_LAYOUT_FIELDS, 0))
            cache.update(
                collision_chain_steps=7,
                metadata_reject_count=6,
                key_comparison_count=3,
                lock_hold_nanoseconds=30,
                max_lock_hold_nanoseconds=12,
                rehash_nanoseconds=5,
                max_rehash_nanoseconds=3,
                arena_refill_count=1,
                arena_refill_nanoseconds=4,
                max_arena_refill_nanoseconds=4,
            )
            self.assertEqual(parse(), telemetry)
            for name in benchmark.SHARED_CACHE_LAYOUT_FIELDS:
                original = cache.pop(name)
                with (
                    self.subTest(missing=name),
                    self.assertRaisesRegex(benchmark.BenchmarkError, "layout counters"),
                ):
                    parse()
                for invalid in (-1, True, 1.5, None):
                    cache[name] = invalid
                    with (
                        self.subTest(counter=name, invalid=invalid),
                        self.assertRaisesRegex(
                            benchmark.BenchmarkError, "layout counters"
                        ),
                    ):
                        parse()
                cache[name] = original
            for name, invalid, message in (
                ("metadata_reject_count", 5, "probe counters"),
                ("key_comparison_count", 4, "probe counters"),
                ("max_lock_hold_nanoseconds", 31, "lock_hold timing"),
                ("max_rehash_nanoseconds", 6, "rehash timing"),
                ("max_arena_refill_nanoseconds", 5, "arena_refill timing"),
                ("rehash_nanoseconds", 10, "nested timing"),
                ("growth_nanoseconds", 31, "nested timing"),
                ("arena_refill_nanoseconds", 31, "nested timing"),
                ("arena_refill_count", 0, "without refill"),
                ("growth_count", 0, "without growth event"),
            ):
                original = cache[name]
                cache[name] = invalid
                with (
            ...
            cache.update(
                growth_count=0,
                rehashed_entries=0,
                rehash_nanoseconds=0,
                max_rehash_nanoseconds=0,
                growth_nanoseconds=1,
                max_growth_nanoseconds=1,
            )
            with self.assertRaisesRegex(
                benchmark.BenchmarkError, "without growth event"
            ):
                parse()
            cache.update(growth_nanoseconds=0, max_growth_nanoseconds=0)
            self.assertEqual(parse(), telemetry)

    def test_search_profile_groups_are_validated_by_telemetry_parser(self) -> None:
        with tempfile.TemporaryDirectory() as directory_name:
            ...
            path = fixture.with_name(fixture.stem + "Telemetry.json")
            serial = benchmark.parse_search_telemetry(path)
            for parallel in (False, True):
                telemetry = json.loads(json.dumps(serial))
                if parallel:
                    self.add_valid_parallel_telemetry(telemetry)
                records = [telemetry]
                if parallel:
                    records.extend(
                        (
                            telemetry["parallel"]["aggregate"],
                            *telemetry["parallel"]["workers"],
                        )
                    )
                for record in records:
                    record["counters"].update(
                        dict.fromkeys(search_profiles.STATE_COUNTERS, 0)
                    )
                path.write_text(json.dumps(telemetry), encoding="utf-8")
                self.assertEqual(benchmark.parse_search_telemetry(path), telemetry)
                records[-1]["counters"]["states_expanded"] = -1
                path.write_text(json.dumps(telemetry), encoding="utf-8")
                with (
                    self.subTest(parallel=parallel),
                    self.assertRaisesRegex(
                        benchmark.BenchmarkError,
                        "invalid search profile.*counter group",
                    ),
                ):
